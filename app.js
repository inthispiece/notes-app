(function () {
  ...
  function initApp(documentRef, storage) {
    const elements = {
      ...
      selectionOverlayCanvas: documentRef.getElementById("selectionOverlayCanvas"),
    ...
    const handwritingContext = elements.handwritingCanvas.getContext
      ...
      : null;
    const selectionOverlayContext = elements.selectionOverlayCanvas?.getContext
      ? elements.selectionOverlayCanvas.getContext("2d")
      : null;
    ...
    function resizeCanvas() {
      ...
      if (elements.selectionOverlayCanvas && selectionOverlayContext) {
        elements.selectionOverlayCanvas.width = canvas.width;
        elements.selectionOverlayCanvas.height = canvas.height;
        selectionOverlayContext.setTransform(ratio, 0, 0, ratio, 0, 0);
      }
      applyDrawingStyle();
    }
    ...
    function clearSelectionOverlay() {
      ...
      const ratio = window.devicePixelRatio || 1;
      selectionOverlayContext.setTransform(1, 0, 0, 1, 0, 0);
      selectionOverlayContext.clearRect(
        0,
        0,
        elements.selectionOverlayCanvas.width,
        elements.selectionOverlayCanvas.height
      );
      selectionOverlayContext.setTransform(ratio, 0, 0, ratio, 0, 0);
    }
    ...
    function drawLassoPath() {
      clearSelectionOverlay();
      if (lassoPoints.length < 2 || !selectionOverlayContext) {
        return;
      }
      const previousOperation = selectionOverlayContext.globalCompositeOperation;
      const previousStroke = selectionOverlayContext.strokeStyle;
      const previousWidth = selectionOverlayContext.lineWidth;
      selectionOverlayContext.globalCompositeOperation = "source-over";
      selectionOverlayContext.strokeStyle = "#2f6fed";
      selectionOverlayContext.lineWidth = 1.5;
      selectionOverlayContext.beginPath();
      selectionOverlayContext.moveTo(lassoPoints[0].x, lassoPoints[0].y);
      lassoPoints.slice(1).forEach((point) => selectionOverlayContext.lineTo(point.x, point.y));
      selectionOverlayContext.stroke();
      selectionOverlayContext.globalCompositeOperation = previousOperation;
      selectionOverlayContext.strokeStyle = previousStroke;
      selectionOverlayContext.lineWidth = previousWidth;
    }
    ...
    function drawSelection(showOutline = true) {
      if (!selection || !selectionOverlayContext) {
        return;
      }
      clearSelectionOverlay();
      const previousOperation = selectionOverlayContext.globalCompositeOperation;
      ...
      const previousWidth = selectionOverlayContext.lineWidth;
      selectionOverlayContext.globalCompositeOperation = "source-over";
      selectionOverlayContext.drawImage(selection.image, selection.x, selection.y, selection.width, selection.height);
      if (showOutline && selection.pathPoints.length > 1) {
        selectionOverlayContext.strokeStyle = "#2f6fed";
        selectionOverlayContext.lineWidth = 1.5;
        traceSelectionPath(selectionOverlayContext, selection);
        selectionOverlayContext.stroke();
      }
      selectionOverlayContext.globalCompositeOperation = previousOperation;
      selectionOverlayContext.strokeStyle = previousStroke;
